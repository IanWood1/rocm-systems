...
from benchmarks.catalog import TRITON_CASES, validated_triton_case
...
def _cpu_random(
    shape: tuple[int, ...], dtype: torch.dtype, *, salt: int = 0
) -> torch.Tensor:
...
def _run_softmax(
    ...
    input_cpu = _cpu_random((rows, columns), torch.float16)
...
def _run_rmsnorm(
    ...
    parameters = validated_triton_case("triton.rmsnorm_bf16")
    rows = parameters["rows"]
    columns = parameters["columns"]
    epsilon = parameters["epsilon"]
    input_cpu = _cpu_random((rows, columns), torch.bfloat16)
    input_gpu = input_cpu.cuda()
    output_gpu = torch.empty_like(input_gpu)
...
def _run_gemm(
    ...
    left_cpu = _cpu_random((rows, reduction), torch.bfloat16)
    right_cpu = _cpu_random((reduction, columns), torch.bfloat16, salt=0xB)
    left_gpu = left_cpu.cuda()
    right_gpu = right_cpu.cuda()
...
def _run_attention(
    ...
    parameters = validated_triton_case("triton.attention_fp16")
    batch = parameters["batch"]
    heads = parameters["heads"]
    sequence = parameters["sequence"]
    head_dimension = parameters["head_dimension"]
    shape = (batch, heads, sequence, head_dimension)
    query_cpu = _cpu_random(shape, torch.float16)
    key_cpu = _cpu_random(shape, torch.float16, salt=0xB)
    value_cpu = _cpu_random(shape, torch.float16, salt=0xC)
...
def main() -> int:
    ...
    if not _target_matches(reported_target, arguments.expected_target):
        raise RuntimeError(
            f"runtime reported target {reported_target!r}, expected {arguments.expected_target!r}"
        )
    ...
    if arguments.case.startswith("triton.softmax"):
        ...
        parameters, durations = _run_rmsnorm(
        ...
        parameters, durations = _run_attention(
            arguments.warmups, arguments.samples
        )
    ...
    result = {
        ...
        "target": {"expected": arguments.expected_target, "reported": reported_target},
        "seed": "0x524a4954",
        "warmups": arguments.warmups,
        "samples": arguments.samples,
        "synchronized_dispatch_ns": durations,
        "workload": parameters,
        "provenance": {
            "adapter": "triton_python",
